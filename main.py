...
class SRTCompareApp(QMainWindow):
    ...
    def init_ui(self):
        """Khởi tạo giao diện"""
        self.setWindowTitle("Reup Tool - So sánh file phụ đề và chỉnh sửa audio")
    ...
    def create_content_area(self) -> QWidget:
        ...
        # Tạo các trang (không còn trang Trạng thái riêng)
    ...
    def create_compare_page(self) -> QWidget:
        ...
        auto_fix_group.setStyleSheet(compare_group.styleSheet())
        ...
        compare_layout.addWidget(auto_fix_group)
        ...
        progress_group.setStyleSheet(compare_group.styleSheet())
        ...
        results_group.setStyleSheet(compare_group.styleSheet())
        ...
        clear_log_btn = QPushButton("🗑️ Làm sạch Log")
        clear_log_btn.clicked.connect(self.clear_log)
        save_log_btn = QPushButton("💾 Lưu Log")
    ...
    def create_settings_page(self) -> QWidget:
        ...
        settings_label = QLabel("Cài đặt")
        settings_label.setStyleSheet("font-size: 22px; font-weight: bold;")
        ...
        theme_info = QLabel("Chọn chế độ giao diện:")
    ...
    def create_info_page(self) -> QWidget:
        ...
        info_text = """
        <h2>Reup Tool</h2>
        <p>Ứng dụng so sánh và chỉnh sửa file phụ đề SRT</p>
        <h3>Chức năng:</h3>
        <ul>
            <li>So sánh 2 file SRT để kiểm tra lệch thời gian</li>
            <li>Tạo file thaisub từ file nguồn</li>
            <li>Mở nhanh file phụ đề bằng ứng dụng mặc định</li>
            <li>Tự động đồng bộ thời gian giữa 2 file phụ đề</li>
        </ul>
        <h3>Chia phụ đề SRT</h3>
        <p>Hỗ trợ chia nhỏ file phụ đề để dễ copy:</p>
        <ul>
            <li>Chọn một file SRT bất kỳ</li>
            <li>Nhập số phụ đề mỗi nhóm (ví dụ 400, 500)</li>
            <li>Tool sẽ chia thành nhiều nhóm, bạn chọn từng nhóm để copy nhanh nội dung</li>
        </ul>
        <h3>Hướng dẫn nhanh:</h3>
        <p>1. Vào tab "So sánh SRT" để so sánh và tạo file thaisub.</p>
        <p>2. Vào tab "Chia phụ đề" để chia nhỏ file SRT thành nhiều phần.</p>
        """
    ...
    def start_compare(self):
        ...
        self.progress_bar.setValue(0)
        self.progress_label.setText("Đang so sánh...")
        ...
        self.compare_thread.progress.connect(self.progress_bar.setValue)
    ...
    def on_compare_finished(self, result: dict):
        ...
        if result['errors']:
            self.log_message(f"Phát hiện {len(result['errors'])} lỗi lệch thời gian")
    ...
    def create_thaisub(self):
        ...
        try:
            ...
            # Xác định số lượng entries cần tạo
            # Nếu có File 2, chỉ lấy số lượng bằng File 2
            target_count = len(source_entries)
            if self.file2_path and self.file2_path.exists():
                try:
                    file2_entries = parse_srt_file(self.file2_path)
                    if file2_entries:
                        target_count = len(file2_entries)
                        self.log_message(f"File 2 có {target_count} entries, chỉ tạo {target_count} entries cho file thaisub")
                except:
                    pass
            
            # Chỉ lấy số lượng entries cần thiết
            entries_to_use = source_entries[:target_count]
    ...
    def open_file1_in_notepad(self):
        """Mở File 1 bằng ứng dụng mặc định của hệ điều hành"""
    ...
    def open_file2_in_notepad(self):
        """Mở File 2 (hoặc file thaisub) bằng ứng dụng mặc định của hệ điều hành"""
    ...
    def capcut_run_trim_audio(self):
        ...
        silence_thresh = int(self.capcut_silence_thresh_spin.value())
        min_silence_len = int(self.capcut_min_silence_spin.value())
    ...
    def capcut_run_align_audio_to_subtitles(self):
        ...
        align_mode = self.capcut_align_mode_combo.currentIndex()
